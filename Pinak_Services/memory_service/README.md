...
The legacy `/search` and `/rag/search` endpoints accept `k`/`limit` from 1 through 100; direct hybrid-search callers are clamped to that range. Hybrid recall now merges semantic, episodic, procedural vectors and keywords with RAG and working-memory literal keyword hits. Working hits exclude expired rows; neither RAG nor working is vector-embedded. The working-add endpoint still runs risk nudges synchronously and can be slow if the embedding model is slow; moving nudges to a queue would change response timing and needs a separate design. Timed vector searches use a bounded shared worker pool; if a query times out or the pool is full, retrieval falls back to keyword matches. This prevents an unbounded pool of workers but does not cancel an already-running embedding operation.
